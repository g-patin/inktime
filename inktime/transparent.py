...
__all__ = ['transmission', 'kaleidoscope', 'optical_density', 'primaries', 'palette_plot', 'Stratigraphy']
...
def palette_plot(rgb_img):
    '''Plot normalized optical density map and histogram for *rgb_img*.

    Corners represent the cyan, magenta and yellow primaries. '''
    ...
    fig, [ax, ax1] = plt.subplots(ncols=2, figsize=[9, 6])
    fig.suptitle('Normalized Optical Density or Palette Plot')

    ax.imshow(nod_rgb_img)
    ax.set_title('NOD map')
    ax.set_xticks([])
    ax.set_yticks([])
